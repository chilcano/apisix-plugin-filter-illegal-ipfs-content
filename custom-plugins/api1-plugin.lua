...
--function _M.body_filter(conf, ctx)
    ...
    if previous_api_response_body ~= nil then
        core.log.error("1st Plugin * There is info in the shared-dict")
        ...
        if previous_resp_id_value ~= nil then
            core.log.error("1st Plugin * Printing ID from previous response: ", previous_resp_id_value)
        else
            core.log.error("1st Plugin * The ID from previous response is NULL")
        end 
    else
        core.log.error("1st Plugin * The shared-dict is NULL")
    end 
    ...
    --     if err then
    --         core.log.error("1st Plugin * 503 error")
    --         return 503
    --     end
    --     if body then
    --         core.log.error("1st Plugin * Updating the body with successfully response")
    --         params.body = body
    --     end
    -- end
    ...
    -- if is_odd(tonumber(api1_res_data_id)) then
    --     core.log.error("2nd Plugin * The ID is odd, We are going to call the Random BloodTypes API")
    --     return 404, "The ID is odd"
    --     --upstream_url_fin = "https://random-data-api.com/api/v2/blood_types"
    -- else 
    --     core.log.error("2nd Plugin * The ID is even, We are going to call the Random Banks API")
    --     --upstream_url_fin = conf.conf.ep.url
    -- end
    ...
    -- last_resp, err = httpc:request_uri(conf.ep.url, {
    ...
    core.log.error("1st Plugin * 11111111")

    if not last_resp then
        core.log.error("1st Plugin * 222222222 ", "Failed when requesting ", conf.ep.url, ", error: ", err)
        return 500, "request failed: " .. err
    end

    core.log.error("1st Plugin * 33333333333333")
    ...
    core.log.error("1st Plugin * 444444444")
    core.log.error("1st Plugin * conf.ep.url: ", conf.ep.url)
    core.log.error("1st Plugin * last_resp.status: ", last_resp.status)
    core.log.error("1st Plugin * last_resp.body: ", last_resp.body)
    ...
    core.log.error("1st Plugin * 555555")
